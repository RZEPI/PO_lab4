...
public class SymulatorWycieczki {
    ...
    public void symuluj() {
        ...
        for(pozycjaGrupy = 0; pozycjaGrupy < wycieczka.getLiczbaElementowWycieczki(); pozycjaGrupy++) {
            ...
            for(SluchaczPostepow s : listaSluchaczy)
            {
                s.aktualizujPostep(wycieczka.getElementWycieczki(pozycjaGrupy), pozycjaGrupy++, wycieczka.getLiczbaElementowWycieczki());
            }
            ...
            {
                Atrakcja atr = (Atrakcja) elementWycieczki;

                System.out.println("Atrakcja odwiedzona przez grupę: " + atr.getNazwa());
                for(Uczestnik u : grupa.getUczestnicy())
                {
                    u.reagujNaAtrakcje(atr, czasWycieczki);
                }
                System.out.println("Atrakcja zajęła: " +atr.getCzasZwiedzania() + " h");
                czasWycieczki += atr.getCzasZwiedzania();
            }
